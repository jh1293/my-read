        ...
        <div className="list-books-content">
          <div>
            <BookShelf title="Currently Reading">
              {this.state.books.filter(book => book.status === 100).map(book => (<Book key={book.title} title={book.title} author={book.author} cover={book.cover} />))}
            </BookShelf>
            <BookShelf title="Want to Read">
              {this.state.books.filter(book => book.status === 200).map(book => (<Book key={book.title} title={book.title} author={book.author} cover={book.cover} />))}
            </BookShelf>
            <BookShelf title="Read">
              {this.state.books.filter(book => book.status === 300).map(book => (<Book key={book.title} title={book.title} author={book.author} cover={book.cover} />))}
            </BookShelf>
          </div>
        </div>
